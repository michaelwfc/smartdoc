...
from configs.config import LLM_DEVICE, LLM_MODEL,  LORA_MODEL_PATH_BAICHUAN
                ...
                if num_gpus < 2 and self.device_map is None:
                    ...
                    if LORA_MODEL_PATH_BAICHUAN:
                        if LLM_MODEL == "Baichuan-13B-Chat":
                            ...
                            from configs.config import LLM_DEVICE, LORA_MODEL_PATH_BAICHUAN
                            # if LORA_MODEL_PATH_BAICHUAN is not None:
                            if LORA_MODEL_PATH_BAICHUAN:
...
def loaderLLM(llm_model: str = None, no_remote_model: bool = False, use_ptuning_v2: bool = False,
              loaderCheckPoint: LoaderCheckPoint = None) -> Any:
    """
    ...
    modelInsLLM = ChatGLMLLMChain(checkPoint=loaderCheckPoint)
    if 'FastChatOpenAILLM' in llm_model_info["provides"]:
